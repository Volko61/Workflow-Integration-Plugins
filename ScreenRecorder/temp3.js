async function getBestAudioDevice() {
    try {
        ...
        // Prefer system audio devices (for recording PC audio)
        const systemAudioDevice = audioDevices.find(device => device.type === 'system');
        if (systemAudioDevice) {
            debugLog(`Using system audio device: ${systemAudioDevice.name}`);
            return systemAudioDevice.name;
        }

        // Fallback to first available microphone
        const microphoneDevice = audioDevices.find(device => device.type === 'microphone');
        if (microphoneDevice) {
            debugLog(`Using microphone device: ${microphoneDevice.name}`);
            return microphoneDevice.name;
        }
        ...
        if (options.sourceType === 'camera') {
            ...
            if (cleanCameraName.toLowerCase().includes('obs virtual')) {
                ...
                getBestAudioDevice().then(audioDevice => {
                    ...
                    debugLog(`Error getting audio device, proceeding with video only: ${error.message}`);
                    const cameraCommand = `${getFFmpegPath()} -f dshow -framerate 60 -i video="${cleanCameraName}" -c:v libx264 -preset ultrafast -crf 22 -pix_fmt yuv420p -flush_packets 1 -vf scale=${options.resolution || '1920x1080'} "${outputPath}"`;
                    debugLog(`OBS Virtual Camera video only (audio error): ${cameraCommand}`);
                    recordingProcess = spawn(cameraCommand, [], { shell: true });
                });
            } else {
